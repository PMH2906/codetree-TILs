...
public class Main {
	...
	static public class MovePoint implements Comparable<MovePoint>{
		...
		public MovePoint(int x, int y, int dist, int direct) {
			super();
			this.x = x;
			this.y = y;
			this.dist = dist;
...
//			}
			...
			while(pq.size()>0) {
				...
				for(int innerD=0;innerD<4;innerD++) {
					int nx=now.x+deltas[innerD][0];
					int ny=now.y+deltas[innerD][1];
					...
					// 다음 이동 위치 선택 
					if(!isNonpass[nx][ny]&&!visited[nx][ny]) {
						if(now.dist==0) pq.add(new MovePoint(nx, ny, now.dist+1,innerD));
						else pq.add(new MovePoint(nx, ny, now.dist+1,now.direct));
						visited[nx][ny]=true;
					}
				}
			}
